...
void av_set_hls_key_and_iv(char *key, char* iv){
    if(key == NULL && iv == NULL){
	s_bSetKey  =0;
 	av_log(NULL, AV_LOG_ERROR, "Disable HLS Using Cuntom Key\n");
    }else {
        s_bSetKey = 1;
 	av_log(NULL, AV_LOG_ERROR, "Enable HLS Using Cuntom Key[%s] [%s]\n", s_key,s_iv);
        if (key) {
            strcpy(s_key, key);
        }
        if (iv) {
...
static int open_input(HLSContext *c, struct playlist *pls, struct segment *seg, AVIOContext **in)
{
	...
	strcpy(iv,    s_iv);
	strcpy(key, s_key);
    	//av_log(NULL, AV_LOG_ERROR, "---- HLS Using Cuntom Key[%s] [%s]\n", s_key,s_iv);
        }else{
    	...
    	//av_log(NULL, AV_LOG_ERROR, "++++ HLS Using File Key[%s] [%s]\n", key, iv);
        }
        ...
        av_log(NULL, AV_LOG_ERROR, "++++ open_url[%s] Key[%s] Iv[%s]  = %d\n", url, key, iv, ret);
