...
PixarLogPreDecode(TIFF* tif, uint16 s)
{
	...
	sp->stream.avail_in = (uint32_t) tif->tif_rawcc;
...
PixarLogDecode(TIFF* tif, uint8* op, tmsize_t occ, uint16 s)
{
	...
	assert(sizeof(sp->stream.avail_out)==4);  /* if this assert gets raised,
	    ...
	    to deal with 8byte memory sizes, though this code will respond
	    appropriately even before we simplify it */
	sp->stream.avail_out = (uint32_t) (nsamples * sizeof(uint16));
...
PixarLogPreEncode(TIFF* tif, uint16 s)
{
	...
	assert(sizeof(sp->stream.avail_out)==4);  /* if this assert gets raised,
	    ...
	    to deal with 8byte memory sizes, though this code will respond
	    appropriately even before we simplify it */
	sp->stream.avail_out = (uint32_t)tif->tif_rawdatasize;
...
PixarLogEncode(TIFF* tif, uint8* bp, tmsize_t cc, uint16 s)
{
	...
	assert(sizeof(sp->stream.avail_in)==4);  /* if this assert gets raised,
	    ...
	    to deal with 8byte memory sizes, though this code will respond
	    appropriately even before we simplify it */
	sp->stream.avail_in = (uint32_t) (n * sizeof(uint16));
	if ((sp->stream.avail_in / sizeof(uint16)) != (uint32_t) n)
	{
		TIFFErrorExt(tif->tif_clientdata, module,
	...
	do {
		...
		if (sp->stream.avail_out == 0) {
			...
			TIFFFlushData1(tif);
			sp->stream.next_out = tif->tif_rawdata;
			sp->stream.avail_out = (uint32_t) tif->tif_rawdatasize;  /* this is a safe typecast, as check is made already in PixarLogPreEncode */
		}
