          ...
          if (coef->iMCU_row_num < last_iMCU_row ||
            ...
            if (blockcnt < compptr->MCU_width) {
              /* Create some dummy blocks at the right edge of the image. */
                moz_jzero_far((void *)coef->MCU_buffer[blkn + blockcnt],
            ...
            /* Create a row of dummy blocks at the bottom of the image. */
              moz_jzero_far((void *)coef->MCU_buffer[blkn],
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    for (block_row = 0; block_row < block_rows; block_row++) {
      ...
      if (ndummy > 0) {
        ...
        moz_jzero_far((void *)thisblockrow, ndummy * sizeof(JBLOCK));
    ...
    if (coef->iMCU_row_num == last_iMCU_row) {
      ...
      for (block_row = block_rows; block_row < compptr->v_samp_factor;
        ...
        moz_jzero_far((void *)thisblockrow,
...
#ifdef C_ARITH_CODING_SUPPORTED
    if (cinfo->arith_code)
        moz_jget_arith_rates(cinfo, compptr->dc_tbl_no, compptr->ac_tbl_no, arith_r);
    else
#endif
    {
        moz_jpeg_make_c_derived_tbl(cinfo, TRUE, compptr->dc_tbl_no, &dctbl);
        moz_jpeg_make_c_derived_tbl(cinfo, FALSE, compptr->ac_tbl_no, &actbl);
    }
...
#ifdef C_ARITH_CODING_SUPPORTED
      if (cinfo->arith_code)
          moz_quantize_trellis_arith(cinfo, arith_r, thisblockrow,
...
#endif
        moz_quantize_trellis(cinfo, dctbl, actbl, thisblockrow,
                         buffer_dst[block_row], blocks_across,
                         cinfo->quant_tbl_ptrs[compptr->quant_tbl_no],
                         cinfo->master->norm_src[compptr->quant_tbl_no],
                         cinfo->master->norm_coef[compptr->quant_tbl_no],
...
GLOBAL(void)
moz_jinit_c_coef_controller(j_compress_ptr cinfo, boolean need_full_buffer)
{
  my_coef_ptr coef;
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
      ...
      coef->whole_image[ci] = (*cinfo->mem->request_virt_barray)
        ((j_common_ptr) cinfo, JPOOL_IMAGE, FALSE,
         (JDIMENSION)moz_jround_up((long) compptr->width_in_blocks,
                                (long) compptr->h_samp_factor),
         (JDIMENSION)moz_jround_up((long) compptr->height_in_blocks,
