...
GLOBAL(void)
moz_jpeg_add_quant_table (j_compress_ptr cinfo, int which_tbl,
 ...
 */
{
  ...
  if (*qtblptr == NULL)
    *qtblptr = moz_jpeg_alloc_quant_table((j_common_ptr) cinfo);
...
GLOBAL(void)
moz_jpeg_default_qtables (j_compress_ptr cinfo, boolean force_baseline)
 ...
 */
{
  /* Set up two quantization tables using the specified scaling */
    moz_jpeg_add_quant_table(cinfo, 0, std_luminance_quant_tbl[cinfo->master->quant_tbl_master_idx],
                       cinfo->q_scale_factor[0], force_baseline);
    moz_jpeg_add_quant_table(cinfo, 1, std_chrominance_quant_tbl[cinfo->master->quant_tbl_master_idx],
...
GLOBAL(void)
moz_jpeg_set_linear_quality (j_compress_ptr cinfo, int scale_factor,
                         boolean force_baseline)
/* Set or change the 'quality' (quantization) setting, using default tables
...
GLOBAL(int)
moz_jpeg_quality_scaling (int quality)
{
  return moz_jpeg_float_quality_scaling(quality);
}

GLOBAL(float)
moz_jpeg_float_quality_scaling(float quality)
...
GLOBAL(void)
moz_jpeg_set_quality (j_compress_ptr cinfo, int quality, boolean force_baseline)
/* Set or change the 'quality' (quantization) setting, using default tables.
 ...
 */
{
  ...
  quality = moz_jpeg_quality_scaling(quality);

  /* Set up standard quality tables */
  moz_jpeg_set_linear_quality(cinfo, quality, force_baseline);
}
...
GLOBAL(void)
moz_jpeg_set_defaults (j_compress_ptr cinfo)
{
  int i;
  ...
  moz_jpeg_set_quality(cinfo, 75, TRUE);
  ...
  moz_jpeg_default_colorspace(cinfo);
...
GLOBAL(void)
moz_jpeg_default_colorspace (j_compress_ptr cinfo)
{
  switch (cinfo->in_color_space) {
  case JCS_GRAYSCALE:
      moz_jpeg_set_colorspace(cinfo, JCS_GRAYSCALE);
    break;
  case JCS_RGB:
  case JCS_EXT_RGB:
  case JCS_EXT_RGBX:
  ...
  case JCS_EXT_ABGR:
  case JCS_EXT_ARGB:
      moz_jpeg_set_colorspace(cinfo, JCS_YCbCr);
    break;
  case JCS_YCbCr:
      moz_jpeg_set_colorspace(cinfo, JCS_YCbCr);
    break;
  case JCS_CMYK:
      moz_jpeg_set_colorspace(cinfo, JCS_CMYK); /* By default, no translation */
    break;
  case JCS_YCCK:
      moz_jpeg_set_colorspace(cinfo, JCS_YCCK);
    break;
  case JCS_UNKNOWN:
      moz_jpeg_set_colorspace(cinfo, JCS_UNKNOWN);
    break;
  default:
...
GLOBAL(void)
moz_jpeg_set_colorspace (j_compress_ptr cinfo, J_COLOR_SPACE colorspace)
{
...
GLOBAL(void)
moz_jpeg_simple_progression (j_compress_ptr cinfo)
{
  int ncomps;
  int nscans;
  jpeg_scan_info *scanptr;
