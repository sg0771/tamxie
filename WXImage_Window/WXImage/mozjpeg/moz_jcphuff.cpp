  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (gather_statistics) {
      ...
      /* We may do this more than once for a table, but it's not expensive */
        moz_jpeg_make_c_derived_tbl(cinfo, is_DC_band, tbl,
...
encode_mcu_AC_first (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  /* Prepare data */
  entropy->AC_first_prepare(MCU_data[0][0], moz_jpeg_natural_order + cinfo->Ss,
                            Sl, Al, values, bits);
...
encode_mcu_AC_refine (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  /* Prepare data */
  EOBPTR = absvalues +
    entropy->AC_refine_prepare(MCU_data[0][0], moz_jpeg_natural_order + cinfo->Ss,
...
finish_pass_gather_phuff (j_compress_ptr cinfo)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (! did[tbl]) {
      ...
      moz_jpeg_gen_optimal_table(cinfo, *htblptr, entropy->count_ptrs[tbl]);
      did[tbl] = TRUE;
    }
  }
}
...
GLOBAL(void)
moz_jinit_phuff_encoder (j_compress_ptr cinfo)
{
