...
#include "moz_jdhuff.h"             /* Declarations shared with jdhuff.c */
#include <limits.h>
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (is_DC_band) {
      if (cinfo->Ah == 0) {     /* DC refinement needs no table */
        ...
        moz_jpeg_make_d_derived_tbl(cinfo, TRUE, tbl, pdtbl);
      }
    } else {
      tbl = compptr->ac_tbl_no;
      pdtbl = (d_derived_tbl **)(entropy->derived_tbls) + tbl;
      moz_jpeg_make_d_derived_tbl(cinfo, FALSE, tbl, pdtbl);
    ...
    if (EOBRUN > 0)             /* if it's a band of zeroes... */
      ...
      for (k = cinfo->Ss; k <= Se; k++) {
        ...
        if (s) {
          ...
          (*block)[moz_jpeg_natural_order[k]] = (JCOEF)LEFT_SHIFT(s, Al);
        } else {
          if (r == 15) {        /* ZRL */
  ...
  if (!entropy->pub.insufficient_data) {
    ...
    if (EOBRUN == 0) {
      for (; k <= Se; k++) {
         ...
         */
        do {
          thiscoef = *block + moz_jpeg_natural_order[k];
          if (*thiscoef != 0) {
            CHECK_BIT_BUFFER(br_state, 1, goto undoit);
            if (GET_BITS(1)) {
        ...
        if (s) {
          int pos = moz_jpeg_natural_order[k];
...
GLOBAL(void)
moz_jinit_phuff_decoder(j_decompress_ptr cinfo)
{
