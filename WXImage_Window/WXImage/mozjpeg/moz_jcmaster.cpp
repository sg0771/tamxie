...
GLOBAL(void)
moz_jpeg_calc_jpeg_dimensions (j_compress_ptr cinfo)
...
/* Do computations that are needed before master selection phase */
{
  ...
  if (!transcode_only)
#endif
      moz_jpeg_calc_jpeg_dimensions(cinfo);
#endif
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    compptr->width_in_blocks = (JDIMENSION)
        moz_jdiv_round_up((long) cinfo->_jpeg_width * (long) compptr->h_samp_factor,
                    (long) (cinfo->max_h_samp_factor * DCTSIZE));
    compptr->height_in_blocks = (JDIMENSION)
        moz_jdiv_round_up((long) cinfo->_jpeg_height * (long) compptr->v_samp_factor,
  ...
  cinfo->total_iMCU_rows = (JDIMENSION)
      moz_jdiv_round_up((long) cinfo->_jpeg_height,
                  (long) (cinfo->max_v_samp_factor*DCTSIZE));
}
...
/* cinfo->comps_in_scan and cinfo->cur_comp_info[] are already set */
{
  ...
  if (cinfo->comps_in_scan == 1) {
    ...
    cinfo->MCUs_per_row = (JDIMENSION)
        moz_jdiv_round_up((long) cinfo->_jpeg_width,
                    (long) (cinfo->max_h_samp_factor*DCTSIZE));
...
prepare_for_pass (j_compress_ptr cinfo)
{
  ...
  case output_pass:
    ...
    if (cinfo->master->optimize_scans) {
      ...
      moz_jpeg_mem_dest_internal(cinfo, &master->scan_buffer[master->scan_number],
          &master->scan_size[master->scan_number], JPOOL_IMAGE);
...
GLOBAL(void)
moz_jinit_c_master_control (j_compress_ptr cinfo, boolean transcode_only)
{
  my_master_ptr master = (my_master_ptr) cinfo->master;
