                   ...
                   int output_rows)
{
  ...
  for (row = input_rows; row < output_rows; row++) {
      moz_jcopy_sample_rows(image_data, input_rows - 1, image_data, row, 1,
                      num_cols);
  }
}
      ...
      if (prep->rows_to_go == cinfo->image_height) {
        for (ci = 0; ci < cinfo->num_components; ci++) {
          ...
          for (row = 1; row <= cinfo->max_v_samp_factor; row++) {
              moz_jcopy_sample_rows(prep->color_buf[ci], 0, prep->color_buf[ci],
...
GLOBAL(void)
moz_jinit_c_prep_controller(j_compress_ptr cinfo, boolean need_full_buffer)
{
  my_prep_ptr prep;
  int ci;
  jpeg_component_info *compptr;
