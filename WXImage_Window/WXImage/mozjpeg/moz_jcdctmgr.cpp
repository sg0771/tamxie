...
#include "moz_jsimddct.h"
#include "moz_jchuff.h"
#include <assert.h>
#include <math.h>
...
preprocess_deringing(DCTELEM *data, const JQUANT_TBL *quantization_table)
{
  ...
  do {
    ...
    if (data[moz_jpeg_natural_order[n]] < maxsample) {
      n++;
      continue;
    }
    ...
    start = n;
    while(++n < size && data[moz_jpeg_natural_order[n]] >= maxsample) {}
    end = n;
    ...
    f1 = data[moz_jpeg_natural_order[start >= 1 ? start-1 : 0]];
    f2 = data[moz_jpeg_natural_order[start >= 2 ? start-2 : 0]];

    l1 = data[moz_jpeg_natural_order[end < size-1 ? end : size-1]];
    l2 = data[moz_jpeg_natural_order[end < size-2 ? end+1 : size-1]];
    ...
    for(i = start; i < end; i++, position += step) {
      ...
      data[moz_jpeg_natural_order[i]] = MIN(tmp, maxovershoot);
    }
    n++;
  }
  while(n < size);
...
#include "moz_jpeg_nbits_table.h"
...
GLOBAL(void)
moz_quantize_trellis(j_compress_ptr cinfo, c_derived_tbl *dctbl, c_derived_tbl *actbl, JBLOCKROW coef_blocks, JBLOCKROW src, JDIMENSION num_blocks,
                 ...
                 JBLOCKROW coef_blocks_above, JBLOCKROW src_above)
{
  ...
  for (bi = 0; bi < num_blocks; bi++) {
    ...
    /* Do AC coefficients */
    for (i = Ss; i <= Se; i++) {
      int z = moz_jpeg_natural_order[i];
      ...
      for (j = Ss-1; j < i; j++) {
        int zz = moz_jpeg_natural_order[j];
  ...
  if (cinfo->master->trellis_eob_opt) {
    ...
    while (bi >= 0) {
      while (bi > last_block) {
        for (j = Ss; j <= Se; j++) {
          int z = moz_jpeg_natural_order[j];
          coef_blocks[bi][z] = 0;
        }
        bi--;
      }
...
GLOBAL(void)
moz_quantize_trellis_arith(j_compress_ptr cinfo, arith_rates *r, JBLOCKROW coef_blocks, JBLOCKROW src, JDIMENSION num_blocks,
                 JQUANT_TBL * qtbl, double *norm_src, double *norm_coef, JCOEF *last_dc_val,
                 JBLOCKROW coef_blocks_above, JBLOCKROW src_above)
{
  int i, j, k, l;
...
GLOBAL(void)
moz_jinit_forward_dct (j_compress_ptr cinfo)
{
  my_fdct_ptr fdct;
  int i;
  ...
  case JDCT_FLOAT:
    ...
    if (jsimd_can_fdct_float())
      ...
      fdct->float_dct = moz_jpeg_fdct_float;
    break;
#endif
  default:
    ERREXIT(cinfo, JERR_NOT_COMPILED);
