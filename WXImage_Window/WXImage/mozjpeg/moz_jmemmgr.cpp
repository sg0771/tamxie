...
#include "moz_jmemsys.h"            /* import the system-dependent declarations */
...
/* Allocate a "small" object */
{
  ...
  if (hdr_ptr == NULL) {
    ...
    for (;;) {
      hdr_ptr = (small_pool_ptr)moz_jpeg_get_small(cinfo, min_request + slop);
      if (hdr_ptr != NULL)
        break;
      slop /= 2;
...
/* Allocate a "large" object */
{
  ...
  hdr_ptr = (large_pool_ptr)moz_jpeg_get_large(cinfo, sizeofobject +
...
/* Allocate the in-memory buffers for any unrealized virtual arrays */
{
  ...
  avail_mem = moz_jpeg_mem_available(cinfo, space_per_minheight, maximum_space,
  ...
  for (sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next) {
    if (sptr->mem_buffer == NULL) { /* if not realized yet */
      ...
      if (minheights <= max_minheights) {
        ...
        moz_jpeg_open_backing_store(cinfo, &sptr->b_s_info,
  ...
  for (bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next) {
    if (bptr->mem_buffer == NULL) { /* if not realized yet */
      ...
      if (minheights <= max_minheights) {
        ...
        moz_jpeg_open_backing_store(cinfo, &bptr->b_s_info,
  ...
  if (ptr->first_undef_row < end_row) {
    ...
    if (ptr->pre_zero) {
      ...
      while (undef_row < end_row) {
          moz_jzero_far((void *)ptr->mem_buffer[undef_row], bytesperrow);
        undef_row++;
      }
    } else {
...
free_pool(j_common_ptr cinfo, int pool_id)
{
  ...
  while (lhdr_ptr != NULL) {
    ...
    moz_jpeg_free_large(cinfo, (void *)lhdr_ptr, space_freed);
  ...
  while (shdr_ptr != NULL) {
    ...
    moz_jpeg_free_small(cinfo, (void *)shdr_ptr, space_freed);
    mem->total_space_allocated -= space_freed;
...
self_destruct(j_common_ptr cinfo)
{
  ...
  moz_jpeg_free_small(cinfo, (void *)cinfo->mem, sizeof(my_memory_mgr));
  cinfo->mem = NULL;            /* ensures I will be called only once */

  moz_jpeg_mem_term(cinfo);         /* system-dependent cleanup */
}
...
moz_jinit_memory_mgr(j_common_ptr cinfo)
{
  ...
  max_to_use = moz_jpeg_mem_init(cinfo); /* system-dependent initialization */

  /* Attempt to allocate memory manager's control block */
  mem = (my_mem_ptr)moz_jpeg_get_small(cinfo, sizeof(my_memory_mgr));
