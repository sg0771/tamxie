...
/* Process a DQT marker */
{
  ...
  while (length > 0) {
    ...
    if (cinfo->quant_tbl_ptrs[n] == NULL)
      cinfo->quant_tbl_ptrs[n] = moz_jpeg_alloc_quant_table((j_common_ptr)cinfo);
    ...
    for (i = 0; i < DCTSIZE2; i++) {
      ...
      quant_ptr->quantval[moz_jpeg_natural_order[i]] = (UINT16)tmp;
    }
...
GLOBAL(boolean)
moz_jpeg_resync_to_restart(j_decompress_ptr cinfo, int desired)
{
...
GLOBAL(void)
moz_jinit_marker_reader(j_decompress_ptr cinfo)
{
...
GLOBAL(void)
moz_jpeg_save_markers(j_decompress_ptr cinfo, int marker_code,
...
GLOBAL(void)
moz_jpeg_set_marker_processor(j_decompress_ptr cinfo, int marker_code,
