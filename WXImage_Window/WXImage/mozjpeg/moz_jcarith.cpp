...
arith_encode(j_compress_ptr cinfo, unsigned char *st, int val)
{
   ...
   */
  sv = *st;
  qe = moz_jpeg_aritab[sv & 0x7F];  /* => Qe_Value */
...
encode_mcu_AC_first(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (ke = cinfo->Se; ke > 0; ke--)
     ...
     */
    if ((v = (*block)[moz_jpeg_natural_order[ke]]) >= 0) {
      if (v >>= cinfo->Al) break;
    } else {
      v = -v;
      if (v >>= cinfo->Al) break;
  ...
  for (k = cinfo->Ss; k <= ke; k++) {
    ...
    for (;;) {
      if ((v = (*block)[moz_jpeg_natural_order[k]]) >= 0) {
        if (v >>= cinfo->Al) {
...
encode_mcu_AC_refine(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (kex = ke; kex > 0; kex--)
    if ((v = (*block)[moz_jpeg_natural_order[kex]]) >= 0) {
      if (v >>= cinfo->Ah) break;
    } else {
      v = -v;
      if (v >>= cinfo->Ah) break;
...
encode_mcu(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    for (ke = DCTSIZE2 - 1; ke > 0; ke--)
      if ((*block)[moz_jpeg_natural_order[ke]]) break;

    /* Figure F.5: Encode_AC_Coefficients */
    for (k = 1; k <= ke; k++) {
      st = entropy->ac_stats[tbl] + 3 * (k - 1);
      arith_encode(cinfo, st, 0);       /* EOB decision */
      while ((v = (*block)[moz_jpeg_natural_order[k]]) == 0) {
        arith_encode(cinfo, st + 1, 0); st += 3; k++;
      }
      arith_encode(cinfo, st + 1, 1);
      /* Figure F.6: Encoding nonzero value v */
...
GLOBAL(void)
moz_jinit_arith_encoder(j_compress_ptr cinfo)
{
...
GLOBAL(void)
moz_jget_arith_rates (j_compress_ptr cinfo, int dc_tbl_no, int ac_tbl_no, arith_rates *r)
{
  int i;
  arith_entropy_ptr entropy = (arith_entropy_ptr) cinfo->entropy;
  ...
  for (i = 0; i < DC_STAT_BINS; i++) {
    ...
    int mps_val = state >> 7;
    float prob_lps = (moz_jpeg_aritab[state & 0x7f] >> 16) / 46340.95; /* 32768*sqrt(2) */
  ...
  for (i = 0; i < AC_STAT_BINS; i++) {
    ...
    int mps_val = state >> 7;
    float prob_lps = (moz_jpeg_aritab[state & 0x7f] >> 16) / 46340.95;
    float prob_0 = (mps_val) ? prob_lps : 1.0 - prob_lps;
    float prob_1 = 1.0 - prob_0;
