...
extern void GetRects(int* rect, int* nRect);
static struct AVFrame* s_tempFrame = NULL;
static struct SwsContext* s_sws = NULL;
static int s_arrRects[1024];
static int s_nRect = 0;

static int func_fill_frame(SDL_VoutOverlay *overlay, const AVFrame *_frame)
{
    assert(overlay);

    GetRects(s_arrRects, &s_nRect);

    AVFrame *frame = _frame;
    if(s_nRect > 0){
        //去水印处理
        if(s_tempFrame && 
        (frame->width != s_tempFrame->width || frame->height != s_tempFrame->height)){
            av_frame_free(&s_tempFrame);
            s_tempFrame = NULL;
        ...
        if(s_tempFrame == NULL){
	        s_tempFrame = av_frame_alloc();
	        s_tempFrame->format = AV_PIX_FMT_YUV420P;
	        s_tempFrame->width = _frame->width;
	        s_tempFrame->height = _frame->height;
	        av_frame_get_buffer(s_tempFrame, 1);
            if(_frame->format != AV_PIX_FMT_YUV420P && s_sws == NULL){
                s_sws = sws_getContext(_frame->width, _frame->height, _frame->format,
        ...
        if(s_sws != NULL){
            //swscale convert 
            sws_scale(s_sws, (const uint8_t * const*)_frame->data, _frame->linesize, 0,
		        s_tempFrame->height, s_tempFrame->data, s_tempFrame->linesize);
        }else{
            //memcpy
            I420Copy(_frame->data[0], _frame->linesize[0],
				_frame->data[1], _frame->linesize[1],
				_frame->data[2], _frame->linesize[2],
				s_tempFrame->data[0], s_tempFrame->linesize[0],
				s_tempFrame->data[1], s_tempFrame->linesize[1],
				s_tempFrame->data[2], s_tempFrame->linesize[2],
				s_tempFrame->width, s_tempFrame->height);
        }
        int(*logo_rects)[4] = (int(*)[4])s_arrRects;
 	    WXDelogos(s_tempFrame->data, s_tempFrame->linesize, 
        s_tempFrame->data, s_tempFrame->linesize, 
        s_tempFrame->width, s_tempFrame->height, 
        1, 
        logo_rects, s_nRect, 
        0, 0);
