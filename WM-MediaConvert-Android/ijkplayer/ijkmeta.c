            ...
            case AVMEDIA_TYPE_VIDEO: {
                ...
                if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO){ 
                    if (st->sample_aspect_ratio.num > 0 && st->sample_aspect_ratio.den > 0) {
                        ijkmeta_set_int64_l(stream_meta, IJKM_KEY_SAR_NUM, st->sample_aspect_ratio.num);
                        ijkmeta_set_int64_l(stream_meta, IJKM_KEY_SAR_DEN, st->sample_aspect_ratio.den);
                    }else  if(codecpar->sample_aspect_ratio.num > 0 && codecpar->sample_aspect_ratio.den > 0){
