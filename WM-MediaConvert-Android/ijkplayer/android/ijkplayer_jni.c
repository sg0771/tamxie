...
//Add By Tam 2022.08.01
static void IjkMediaPlayer_setHlsKey(JNIEnv *env, jobject thiz, jstring key, jstring iv) {
    const char *c_key = (*env)->GetStringUTFChars(env, key, NULL );
    const char *c_iv = (*env)->GetStringUTFChars(env, iv, NULL );
    ALOGE("av_set_hls_key_and_iv [%s] [%s]", c_key, c_iv);
    if(strlen(c_key) == 0 && strlen(c_iv) == 0){
	    av_set_hls_key_and_iv(NULL,NULL);
    }else{
	    av_set_hls_key_and_iv((char*)c_key, (char*)c_iv);
    }
    (*env)->ReleaseStringUTFChars(env, key, c_key);
    (*env)->ReleaseStringUTFChars(env, iv, c_iv);
}


//static int s_s1 = 0;

static int g_arrRect[1024];
static int g_nRect = 0;
static void SetRects(int* rects, int nRect){
    if(nRect)
        memcpy(g_arrRect, rects, nRect * sizeof(int) * 4);
    else
        memset(g_arrRect, 0, 256 * sizeof(int) * 4);
    g_nRect = nRect;
}
void GetRects(int* rects, int* nRect){
    //if(s_s1 == 0){
     //   s_s1 = 1;
     //   g_nRect = 1;
    //    g_arrRect[0] = 50;
    ...
    *nRect = g_nRect;
    if(nRect  > 0)
        memcpy(rects, g_arrRect, *nRect * sizeof(int) * 4);
}

//private native void _setDelogoRect(int [] logo_rects, int nb_rect);
//Add By Tam 2023.08.09
static void IjkMediaPlayer_setDelogoRect(JNIEnv *env, jobject thiz, jintArray intArray, jint nb_logos) {
    ALOGE("IjkMediaPlayer_setDelogoRect [%d]", nb_logos);
 	jint *logo_rectangles = (*env)->GetIntArrayElements(env, intArray, NULL);
    SetRects(logo_rectangles, nb_logos);
    (*env)->ReleaseIntArrayElements(env, intArray, logo_rectangles, 0);
}
...
static JNINativeMethod g_methods[] = {
    ...
    { "_setHlsKey",        "(Ljava/lang/String;Ljava/lang/String;)V", 
    (void *) IjkMediaPlayer_setHlsKey },
     //Add By Tam 2022.08.01

      { "_setDelogoRect",   "([II)V", (void *) IjkMediaPlayer_setDelogoRect },
     //Add By Tam 2023.08.09
};


extern jint  SDL_OnLoad(JavaVM *vm, void *reserved);
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    if ((*vm)->GetEnv(vm, (void**) &env, JNI_VERSION_1_4) != JNI_OK) {
        ALOGE("JNI_OnLoad GetEnv 1.4 Error");
        return -1;
    }

    assert(env != NULL);//env error
    SDL_OnLoad(vm, reserved);
