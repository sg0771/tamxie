...
static  av_const int ff_log2_x86(unsigned int v)
{
    unsigned long n;
...
static  av_const unsigned av_mod_uintp2_bmi2(unsigned a, unsigned p)
{
...
static  av_const double av_clipd_sse2(double a, double amin, double amax)
{
...
static  av_const float av_clipf_sse(float a, float amin, float amax)
{
#if defined(ASSERT_LEVEL) && ASSERT_LEVEL >= 2
    if (amin > amax) abort();
#endif
