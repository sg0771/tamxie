...
static  SoftFloat autocorr_calc(int64_t accu)
{
        int nz, mant, expo;
        unsigned round;
        int i = (int)(accu >> 32);
...
static  void autocorrelate(const int x[40][2], SoftFloat phi[3][2][2], int lag)
{
    int i;
...
static  int sbr_hf_apply_noise(int (*Y)[2],
                                                const SoftFloat *s_m,
