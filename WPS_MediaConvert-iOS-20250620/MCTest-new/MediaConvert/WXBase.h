...
class WXString {
	...
	std::wstring ANSIToUnicode(const std::string& str) {
		...
		if (static_cast<size_t>(-1) != len) {
			std::shared_ptr<wchar_t>buff(new wchar_t[len + 1]);
...
class WXThread :public WXTrace {
	...
	std::atomic_bool m_bUseTask = false;//异步任务
...
public:
	...
	void RunTask(WXTask task, int bSync = FALSE) {
		if (!m_bThreadStop.load() && m_bUseTask.load()) {
	...
	void ThreadStop() {
		m_bThreadStop.store(true);//结束线程循环
		if (m_condWait) {
