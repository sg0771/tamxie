...
- (void)Display:(uint8_t *)data{
    [EAGLContext setCurrentContext:m_glContext];
    glBindTexture(GL_TEXTURE_2D, m_textureYUV[TEXY]);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_iVideoWidth, m_iVideoHeight,
                    GL_RED_EXT, GL_UNSIGNED_BYTE, data);
    glBindTexture(GL_TEXTURE_2D, m_textureYUV[TEXU]);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_iVideoWidth/2, m_iVideoHeight/2,
                    GL_RED_EXT, GL_UNSIGNED_BYTE, data + m_iVideoWidth * m_iVideoHeight);
    glBindTexture(GL_TEXTURE_2D, m_textureYUV[TEXV]);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_iVideoWidth/2, m_iVideoHeight/2,
                    GL_RED_EXT, GL_UNSIGNED_BYTE, data + m_iVideoWidth * m_iVideoHeight * 5 / 4);
...
- (void)setVideoSize:(GLuint)width height:(GLuint)height{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED_EXT, width/2, height/2, 0,
                 GL_RED_EXT, GL_UNSIGNED_BYTE, blackData + width * height);
    
    glBindTexture(GL_TEXTURE_2D, m_textureYUV[TEXV]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED_EXT, width/2, height/2, 0,
                 GL_RED_EXT, GL_UNSIGNED_BYTE, blackData + width * height * 5 / 4);
...
class WXVideoRender:public WXThread{
private:
    WXLocker m_mutex;
    UIView *m_view = nil;
    ...
    WXDataBuffer m_arrData[2];
    ThreadSafeQueue<WXDataBuffer*>m_queuePool;
    ThreadSafeQueue<WXDataBuffer*>m_queueData;
    int m_bInit = 0;
...
public:
    ...
    virtual void ThreadProcess(){
        WXDataBuffer* obj = m_queueData.Pop();
        if(obj){
            m_ui ++;
            dispatch_async(dispatch_get_main_queue(), ^{ //UI Thread
                [m_render Display:obj->GetBuffer()];
                m_queuePool.Push(obj);
                m_ui --;
            });
        }
        SLEEPMS(1);
    ...
    virtual void Init(void *view, int w, int h, WXFfmpegOnVideoData cbData){
        WXAutoLock al(m_mutex);
        dispatch_async(dispatch_get_main_queue(), ^{
            m_width = w;
            m_height = h;
            m_cbData = cbData;
            m_arrData[0].Init(nullptr, w*h*3/2);
            m_arrData[1].Init(nullptr, w*h*3/2);
            m_queuePool.Push(&m_arrData[0]);
            m_queuePool.Push(&m_arrData[1]);
            m_view = (__bridge  UIView*)view;
            m_render = [[OpenGLView20 alloc]initWithFrame:CGRectMake(0, 0, m_view.frame.size.width, m_view.frame.size.height)];
    ...
    virtual  void Display(uint8_t** data, int* linesize){
        WXAutoLock al(m_mutex);
        if(!m_bInit)
            return;
        WXDataBuffer* obj = m_queuePool.Pop();
        if(obj){
            int imgsize = m_width * m_height;
            libyuv::I420Copy(data[0], linesize[0],
                data[1], linesize[1],
                data[2], linesize[2],
                obj->GetBuffer(),                    m_width,
                obj->GetBuffer() + imgsize,         m_width / 2,
                obj->GetBuffer() + imgsize * 5 / 4, m_width / 2,
                m_width, m_height);
            if(m_cbData){ //渲染处理，可以在这里去水印
                int size = m_width * m_height;
                uint8_t *data[4] = { obj->GetBuffer(),
                    obj->GetBuffer() + size,
                    obj->GetBuffer() + size * 5/4,
                    NULL
                };
                int linesize[4] = { m_width, m_width/2,m_width/2,0};
                m_cbData(data,linesize,m_width,m_height);
            }
            m_queueData.Push(obj);
        }
    }
};
