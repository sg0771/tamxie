...
static void alv_prosses(uint8_t *buf, int bufsize){
...
static int alv_probe(AVProbeData *p)
{
    return probe(p, 0);
}
...
static void add_keyframes_index(AVFormatContext *s)
{
    ...
    if (alv->last_keyframe_stream_index < 0) {
        av_log(s, AV_LOG_DEBUG, "keyframe stream hasn't been created\n");
        return;
    }

    av_assert0(alv->last_keyframe_stream_index <= s->nb_streams);
    stream = s->streams[alv->last_keyframe_stream_index];

    if (stream->nb_index_entries == 0) {
        for (i = 0; i < alv->keyframe_count; i++) {
            av_log(s, AV_LOG_TRACE, "keyframe filepositions = %"PRId64" times = %"PRId64"\n",
                   alv->keyframe_filepositions[i], alv->keyframe_times[i] * 1000);
            av_add_index_entry(stream, alv->keyframe_filepositions[i],
                alv->keyframe_times[i] * 1000, 0, 0, AVINDEX_KEYFRAME);
        }
    } else
        av_log(s, AV_LOG_WARNING, "Skipping duplicate index\n");

    if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        av_freep(&alv->keyframe_times);
        av_freep(&alv->keyframe_filepositions);
        alv->keyframe_count = 0;
    }
}
...
static AVStream *create_stream(AVFormatContext *s, int codec_type)
{
    ...
    if (codec_type == AVMEDIA_TYPE_AUDIO) {
        st->codecpar->bit_rate = alv->audio_bit_rate;
        alv->missing_streams &= ~FLV_HEADER_FLAG_HASAUDIO;
    }
    if (codec_type == AVMEDIA_TYPE_VIDEO) {
        st->codecpar->bit_rate = alv->video_bit_rate;
        alv->missing_streams &= ~FLV_HEADER_FLAG_HASVIDEO;
        st->avg_frame_rate = alv->framerate;
    }


    avpriv_set_pts_info(st, 32, 1, 1000); /* 32 bit pts in ms */
    alv->last_keyframe_stream_index = s->nb_streams - 1;
    add_keyframes_index(s);
    return st;
}

static int alv_same_audio_codec(AVCodecParameters *apar, int flags)
{
    int bits_per_coded_sample = (flags & FLV_AUDIO_SAMPLESIZE_MASK) ? 16 : 8;
    int alv_codecid           = flags & FLV_AUDIO_CODECID_MASK;
    int codec_id;
    ...
    default:
        return apar->codec_tag == (alv_codecid >> FLV_AUDIO_CODECID_OFFSET);
    }
}

static void alv_set_audio_codec(AVFormatContext *s, AVStream *astream,
                                AVCodecParameters *apar, int alv_codecid)
{
    switch (alv_codecid) {
    // no distinction between S16 and S8 PCM codec flags
    case FLV_CODECID_PCM:
    ...
    default:
        avpriv_request_sample(s, "Audio codec (%x)",
               alv_codecid >> FLV_AUDIO_CODECID_OFFSET);
        apar->codec_tag = alv_codecid >> FLV_AUDIO_CODECID_OFFSET;
    }
}

static int alv_same_video_codec(AVCodecParameters *vpar, int flags)
{
    int alv_codecid = flags & FLV_VIDEO_CODECID_MASK;
    ...
    default:
        return vpar->codec_tag == alv_codecid;
    }
}

static int alv_set_video_codec(AVFormatContext *s, AVStream *vstream,
                               int alv_codecid, int read)
{
    int ret = 0;
    ...
    case FLV_CODECID_VP6A:
        if (alv_codecid == FLV_CODECID_VP6A)
        ...
        ret = 1;     // 1 byte body size adjustment for alv_read_packet()
        break;
    case FLV_CODECID_H264:
    ...
    case FLV_CODECID_MPEG4:
        ...
        break;
    default:
        avpriv_request_sample(s, "Video codec (%x)", alv_codecid);
        par->codec_tag = alv_codecid;
    }
...
static int parse_keyframes_index(AVFormatContext *s, AVIOContext *ioc, int64_t max_pos)
{
    ...
    if (alv->keyframe_count > 0) {
        av_log(s, AV_LOG_DEBUG, "keyframes have been paresed\n");
        return 0;
    }
    av_assert0(!alv->keyframe_times);
    av_assert0(!alv->keyframe_filepositions);
    ...
    if (timeslen == fileposlen && fileposlen>1 && max_pos <= filepositions[0]) {
        for (i = 0; i < FFMIN(2,fileposlen); i++) {
            alv->validate_index[i].pos = filepositions[i];
            alv->validate_index[i].dts = times[i] * 1000;
            alv->validate_count        = i + 1;
        }
        alv->keyframe_times = times;
        alv->keyframe_filepositions = filepositions;
        alv->keyframe_count = timeslen;
        times = NULL;
        filepositions = NULL;
    } else {
invalid:
                            ...
                            int64_t max_pos, int depth)
{
    ...
    if (key) {
        ...
        if (depth == 1) {
            if (amf_type == AMF_DATA_TYPE_NUMBER ||
                ...
                if (!strcmp(key, "duration"))
                    ...
                    alv->video_bit_rate = num_val * 1024.0;
                else if (!strcmp(key, "audiodatarate") &&
                         0 <= (int)(num_val * 1024.0))
                    alv->audio_bit_rate = num_val * 1024.0;
                    ...
                    alv->framerate = av_d2q(num_val, 1000);
                    if (vstream)
                        vstream->avg_frame_rate = alv->framerate;
                } else if (alv->trust_metadata) {
                    if (!strcmp(key, "videocodecid") && vpar) {
                        int ret = alv_set_video_codec(s, vstream, num_val, 0);
                        if (ret < 0)
                            return ret;
                    } else if (!strcmp(key, "audiocodecid") && apar) {
                        int id = ((int)num_val) << FLV_AUDIO_CODECID_OFFSET;
                        alv_set_audio_codec(s, astream, apar, id);
        ...
        if ((!strcmp(key, "duration")        ||
            ...
            !strcmp(key, "datastream")) && !alv->dump_full_metadata)
            return 0;
...
static int alv_read_metabody(AVFormatContext *s, int64_t next_pos)
{
    ALVContext *alv = s->priv_data;
    AMFDataType type;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            vstream = stream;
            alv->last_keyframe_stream_index = i;
        } else if (stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            astream = stream;
            if (alv->last_keyframe_stream_index == -1)
                alv->last_keyframe_stream_index = i;
        }
...
static int alv_read_header(AVFormatContext *s)
{
    int flags;
    ALVContext *alv = s->priv_data;
    int offset;
    ...
    alv->missing_streams = flags & (FLV_HEADER_FLAG_HASVIDEO | FLV_HEADER_FLAG_HASAUDIO);
    ...
    if (pre_tag_size) {
        av_log(s, AV_LOG_WARNING, "Read FLV header error, input file is not a standard alv format, first PreviousTagSize0 always is 0\n");
    }

    s->start_time = 0;
    alv->sum_alv_tag_size = 0;
    alv->last_keyframe_stream_index = -1;

    return 0;
}

static int alv_read_close(AVFormatContext *s)
{
    int i;
    ALVContext *alv = s->priv_data;
    for (i=0; i<FLV_STREAM_TYPE_NB; i++)
        av_freep(&alv->new_extradata[i]);
    av_freep(&alv->keyframe_times);
    av_freep(&alv->keyframe_filepositions);
    return 0;
}

static int alv_get_extradata(AVFormatContext *s, AVStream *st, int size)
{
    if (!size)
        return 0;
	...
	alv_prosses(st->codecpar->extradata,st->codecpar->extradata_size);
    }
    
    return 0;
}

static int alv_queue_extradata(ALVContext *alv, AVIOContext *pb, int stream,
                               int size)
{
    if (!size)
        return 0;

    av_free(alv->new_extradata[stream]);
    alv->new_extradata[stream] = av_mallocz(size +
                                            AV_INPUT_BUFFER_PADDING_SIZE);
    if (!alv->new_extradata[stream])
        return AVERROR(ENOMEM);
    alv->new_extradata_size[stream] = size;
    avio_read(pb, alv->new_extradata[stream], size);
    return 0;
}
...
static int alv_data_packet(AVFormatContext *s, AVPacket *pkt,
...
static int resync(AVFormatContext *s)
{
    ...
    for (i=0; !avio_feof(s->pb); i++) {
        ...
        alv->resync_buffer[j ] =
        alv->resync_buffer[j1] = avio_r8(s->pb);

        if (i >= 8 && pos) {
            uint8_t *d = alv->resync_buffer + j1 - 8;
            if (d[0] == 'F' &&
                d[1] == 'L' &&
                d[2] == 'V' &&
                d[3] < 5 && d[5] == 0) {
                av_log(s, AV_LOG_WARNING, "Concatenated FLV detected, might fail to demux, decode and seek %"PRId64"\n", alv->last_ts);
                alv->time_offset = alv->last_ts + 1;
                alv->time_pos    = avio_tell(s->pb);
            }
        }

        if (i > 22) {
            unsigned lsize2 = AV_RB32(alv->resync_buffer + j1 - 4);
            if (lsize2 >= 11 && lsize2 + 8LL < FFMIN(i, RESYNC_BUFFER_SIZE)) {
                unsigned  size2 = AV_RB24(alv->resync_buffer + j1 - lsize2 + 1 - 4);
                unsigned lsize1 = AV_RB32(alv->resync_buffer + j1 - lsize2 - 8);
                if (lsize1 >= 11 && lsize1 + 8LL + lsize2 < FFMIN(i, RESYNC_BUFFER_SIZE)) {
                    unsigned  size1 = AV_RB24(alv->resync_buffer + j1 - lsize1 + 1 - lsize2 - 8);
...
static int alv_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ALVContext *alv = s->priv_data;
...
retry:
    /* pkt size is repeated at end. skip it */
        ...
        if (alv->validate_next < alv->validate_count) {
            int64_t validate_pos = alv->validate_index[alv->validate_next].pos;
            if (pos == validate_pos) {
                if (FFABS(dts - alv->validate_index[alv->validate_next].dts) <=
                    VALIDATE_INDEX_TS_THRESH) {
                    alv->validate_next++;
                } else {
                    clear_index_entries(s, validate_pos);
                    alv->validate_count = 0;
                }
        ...
        if (type == FLV_TAG_TYPE_AUDIO) {
            ...
            if (size > 13 + 1 + 4) { // Header-type metadata stuff
                ...
                type = alv_read_metabody(s, next);
                if (type == 0 && dts == 0 || type < 0) {
                    if (type < 0 && alv->validate_count &&
                        alv->validate_index[0].pos     > next &&
                        alv->validate_index[0].pos - 4 < next
                    ) {
                        av_log(s, AV_LOG_WARNING, "Adjusting next position due to index mismatch\n");
                        next = alv->validate_index[0].pos - 4;
                    }
                    if (type < 0)
                        alv->broken_sizes = 1;
                    goto skip;
                    ...
                    return alv_data_packet(s, pkt, dts, next);
            ...
            av_log(s, AV_LOG_DEBUG,
                   "Skipping alv packet: type %d, size %d, flags %d.\n",
                   type, size, flags);
skip:
            if (avio_seek(s->pb, next, SEEK_SET) != next) {
                 // This can happen if alv_read_metabody above read past
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ...
            if (stream_type == FLV_STREAM_TYPE_AUDIO) {
                if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
                    (s->audio_codec_id || alv_same_audio_codec(st->codecpar, flags)))
                    break;
            } else if (stream_type == FLV_STREAM_TYPE_VIDEO) {
                if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
                    (s->video_codec_id || alv_same_video_codec(st->codecpar, flags)))
                    break;
...
retry_duration:
    ...
    if (stream_type == FLV_STREAM_TYPE_AUDIO) {
        ...
        if (!st->codecpar->codec_id) {
            alv_set_audio_codec(s, st, st->codecpar,
                                flags & FLV_AUDIO_CODECID_MASK);
            alv->last_sample_rate =
            ...
            alv_set_audio_codec(s, st, par, flags & FLV_AUDIO_CODECID_MASK);
            sample_rate = par->sample_rate;
            avcodec_parameters_free(&par);
        }
    } else if (stream_type == FLV_STREAM_TYPE_VIDEO) {
        int ret = alv_set_video_codec(s, st, flags & FLV_VIDEO_CODECID_MASK, 1);
        if (ret < 0)
            return ret;
        size -= ret;
    } else if (stream_type == FLV_STREAM_TYPE_SUBTITLE) {
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_AAC ||
        ...
        if (type == 0 && (!st->codecpar->extradata || st->codecpar->codec_id == AV_CODEC_ID_AAC ||
            ...
            if (st->codecpar->extradata) {
                if ((ret = alv_queue_extradata(alv, s->pb, stream_type, size)) < 0)
                    return ret;
                ret = FFERROR_REDO;
                goto leave;
            }
            if ((ret = alv_get_extradata(s, st, size)) < 0)
                return ret;
    ...
    pkt->stream_index = st->index;
    pkt->pos          = pos;
    if (alv->new_extradata[stream_type]) {
        uint8_t *side = av_packet_new_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA,
                                                alv->new_extradata_size[stream_type]);
        if (side) {
            memcpy(side, alv->new_extradata[stream_type],
                   alv->new_extradata_size[stream_type]);
            av_freep(&alv->new_extradata[stream_type]);
            alv->new_extradata_size[stream_type] = 0;
        }
    }
    if (stream_type == FLV_STREAM_TYPE_AUDIO &&
                    (sample_rate != alv->last_sample_rate ||
                     channels    != alv->last_channels)) {
        alv->last_sample_rate = sample_rate;
        alv->last_channels    = channels;
...
leave:
    last = avio_rb32(s->pb);
    if (!alv->trust_datasize) {
        if (last != orig_size + 11 && last != orig_size + 10 &&
            !avio_feof(s->pb) &&
            (last != orig_size || !last) && last != alv->sum_alv_tag_size &&
            !alv->broken_sizes) {
            av_log(s, AV_LOG_ERROR, "Packet mismatch %d %d %d\n", last, orig_size + 11, alv->sum_alv_tag_size);
...
static int alv_read_seek(AVFormatContext *s, int stream_index,
...
#define OFFSET(x) offsetof(ALVContext, x)
#define VD AV_OPT_FLAG_VIDEO_PARAM | AV_OPT_FLAG_DECODING_PARAM
static const AVOption options[] = {
    { "alv_metadata", "Allocate streams according to the onMetaData array", OFFSET(trust_metadata), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, VD },
    { "alv_full_metadata", "Dump full metadata of the onMetadata", OFFSET(dump_full_metadata), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, VD },
    { "alv_ignore_prevtag", "Ignore the Size of previous tag", OFFSET(trust_datasize), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, VD },
    { "missing_streams", "", OFFSET(missing_streams), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 0xFF, VD | AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY },
    { NULL }
};

static const AVClass alv_class = {
    .class_name = "alvdec",
...
AVInputFormat ff_alv_demuxer = {
    .name           = "alv",
    .long_name      = NULL_IF_CONFIG_SMALL("ALV (ApowerSoft Video)"),
    .priv_data_size = sizeof(ALVContext),
    .read_probe     = alv_probe,
    .read_header    = alv_read_header,
    .read_packet    = alv_read_packet,
    .read_seek      = alv_read_seek,
    .read_close     = alv_read_close,
    .extensions     = "alv",
    .priv_class     = &alv_class,
};
