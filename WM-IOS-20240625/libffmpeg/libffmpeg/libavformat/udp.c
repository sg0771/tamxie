                                     ...
                                     int nb_sources, int include)
{
	...
	struct sockaddr_in *sockaddr_in_addr = (struct sockaddr_in *)addr;
	struct sockaddr_in *sockaddr_in_local_addr = (struct sockaddr_in *)local_addr;
#ifdef ARCH_AARCH64 
	memcpy(&mreqs.imr_multiaddr,&sockaddr_in_addr->sin_addr,sizeof(struct in_addr));
#else
        mreqs.imr_multiaddr.s_addr = sockaddr_in_addr->sin_addr.s_addr;//arm64
#endif
        if (local_addr)
#ifdef ARCH_AARCH64 
		memcpy(&mreqs.imr_interface, &sockaddr_in_local_addr->sin_addr, sizeof(struct in_addr));
#else
            	mreqs.imr_interface= sockaddr_in_local_addr->sin_addr;//arm64
#endif
        else
#ifdef ARCH_AARCH64 
          memset(&mreqs.imr_interface,0,sizeof(struct in_addr));
#else
	mreqs.imr_interface.s_addr= INADDR_ANY; //arm64
#endif
          
#ifdef ARCH_AARCH64 
        struct sockaddr_in *sockaddr_in_sources= (struct sockaddr_in *)&sources[i];
        memcpy(&mreqs.imr_sourceaddr, &sockaddr_in_sources->sin_addr, sizeof(struct in_addr));
#else
        mreqs.imr_sourceaddr.s_addr = ((struct sockaddr_in *)&sources[i])->sin_addr.s_addr;//arm64
#endif
