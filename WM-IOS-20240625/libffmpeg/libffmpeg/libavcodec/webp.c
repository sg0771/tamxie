 ...
 */
static  int webp_get_vlc(GetBitContext *gb, VLC_TYPE (*table)[2])
{
    int n, nb_bits;
    unsigned int index;
    int code;
...
static  void color_cache_put(ImageContext *img, uint32_t c)
{
...
static  uint8_t clamp_add_subtract_half(int a, int b, int c)
{
    int d = a + b >> 1;
...
static  uint8_t color_transform_delta(uint8_t color_pred,
                                                      uint8_t color)
{
