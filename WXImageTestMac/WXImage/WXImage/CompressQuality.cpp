...
static int _BufferToBuffer(uint8_t* buf, int buf_size,
	...
	else if (image_type < 0 || image_type > WXIMAGE_TYPE_WEBP) {
	...
	if (bitmap.get() == nullptr) {
		...
		//图像编码
		int _image_size = Image_Encode(handle, _image_type, target_level,
			_nTypeRGB, _pDst, _nDstW, _nDstH, _nDstPitch,
			_nDotsPerMeterX, _nDotsPerMeterY,
			_pIccData, _nIccSize);
		if (_image_size <= 0 && _pIccData && _nIccSize > 0) {
			//可能不支持该ICC
			_image_size = Image_Encode(handle, _image_type, target_level,
				_nTypeRGB, _pDst, _nDstW, _nDstH, _nDstPitch,
				_nDotsPerMeterX, _nDotsPerMeterY,
				nullptr, 0);
		}
...
WXIMAGE_API int CompressQuality_FileToFile(const char* strInput, const char* strOutput,
	...
	else if (target_type < 0 || target_type > WXIMAGE_TYPE_WEBP) {
