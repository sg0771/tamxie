...
public:
	...
	void Log(const char* format, ...) {
		...
		//ms_log_callback(NULL, AV_LOG_ERROR, "-- %s -- ", m_szMsg);
	}
...
class MediaConvert {
	...
	int ParserFast(const char* strFileName) {
        ...
        LOGE("[%s] ParserFast AAAA", strFileName);
		...
		if (err < 0) {
			...
			LOGE("[%s] avformat_open_input OK", strFileName);
		}
		...
		if (err < 0) {
			...
			LOGE("[%s] avformat_find_stream_info OK", strFileName);
		}
