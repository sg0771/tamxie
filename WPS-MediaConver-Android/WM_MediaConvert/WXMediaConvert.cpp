...
WXDELOGO_CAPI int64_t WXMediaFfmpeg_Convert(int argc, char **argv);
...
public:
	...
	int LogRet(int ret) {
        ...
        for (int i = 0; i < m_argc; i++) {
			LOGE("m_argv[%d]=%s",i, m_argv[i]);
        }
        WXString wxstr = WXFfmpegGetError(ret);
        LOGE("--- WXConvert Result = %s", wxstr.c_str());
		return ret;
	}
...
public:
        ...
        if (WXStrcmp(Last3, _T(".dv")) == 0) { //dvvideo DV视频格式
			...
			if (m_iWidth != 0 && m_iHeight != 0) { //-s 352x288  分辨率设置
				...
				//设置DAR
				{
					...
					m_argv[m_argc++] = av_strdup("-aspect");//设置输出比例为PAR，使得在MAC分辨率正常
			...
			if (m_strVideoCodec != _T("noset")) { //-vcodec libx264
				...
				if (m_strVideoCodec == _T("h264") || m_strVideoCodec == _T("libx264")){
					m_argv[m_argc++] = av_strdup("-tune");
					m_argv[m_argc++] = av_strdup("zerolatency");

					m_argv[m_argc++] = av_strdup("-c:a");
					m_argv[m_argc++] = av_strdup("copy");
				}
			}
			...
			if (m_strVideoFmt != _T("noset")) { //-pix_fmt yuv420p
				m_argv[m_argc++] = av_strdup("-pix_fmt");
				m_argv[m_argc++] = av_strdup(m_strVideoFmt.c_str());
			...
			m_argv[m_argc++] = av_strdup("-threads"); 
			m_argv[m_argc++] = av_strdup("auto");
	...
	int ShotPicture(WXCTSTR strInput,int64_t ts,WXCTSTR strOutput) {
		...
		m_argv[m_argc++] = av_strdup("ffmpeg");
		m_argv[m_argc++] = av_strdup("-ss");
		m_argv[m_argc++] = av_asprintf("%f", (double)(ts) / 1000.0);//传入毫秒
		m_argv[m_argc++] = av_strdup("-i");
		m_argv[m_argc++] = av_strdup(wxstrInput.c_str());//输入文件
		m_argv[m_argc++] = av_strdup(wxstrOutput.c_str());//输出文件
		m_argv[m_argc++] = av_strdup("-r");
...
WXDELOGO_CAPI void WXFfmpegParamSetPictureQuality(void * p, int brightness, int contrast, int saturation) { //亮度
	WXConvert *obj = (WXConvert *)p;
	if (obj)obj->SetPictureQuality(brightness, contrast, saturation);
}

WXDELOGO_CAPI void *WXFfmpegParamCreate() {
...
WXDELOGO_CAPI void WXFfmpegParamAddWMImage(void * p, WXCTSTR wszImage,int x, int y, int w, int h) {
	WXConvert *obj = (WXConvert *)p;
...
WXDELOGO_CAPI void WXFfmpegParamSetSubtitleAlignment(void * p, int alignment) {
    WXConvert *obj = (WXConvert *)p;
...
WXDELOGO_CAPI int  WXFfmpegCutFile(void * p, WXCTSTR strInput, WXCTSTR strOutput,
	int64_t ptsStart, int64_t ptsDuration, int Fast, int async) {
	WXConvert *obj = (WXConvert *)p;
	return  obj ? obj->CutFile(strInput, strOutput, ptsStart, ptsDuration, Fast, async) : -1;
}

WXDELOGO_CAPI int  WXFfmpegConvertVideo(void * p, WXCTSTR strInput, WXCTSTR strOutput, int async) {
	WXConvert *obj = (WXConvert *)p;
	return  obj ? obj->ConvertVideo(strInput, strOutput, async) : -1;
}

WXDELOGO_CAPI int  WXFfmpegConvertAudio(void * p, WXCTSTR strInput, WXCTSTR strOutput, int async) {
	WXConvert *obj = (WXConvert *)p;
	return  obj ? obj->ConvertAudio(strInput, strOutput, async) : -1;
}

WXDELOGO_CAPI int WXFfmpegMergerFile(void * p, WXCTSTR strOutput,WXCTSTR strTemp, int fast, int async) {
	WXConvert *obj = (WXConvert *)p;
	return obj ? obj->MergerFile(strOutput, strTemp, fast, async) : -1;
}

WXDELOGO_CAPI int WXFfmpegMixerAV(void * p, WXCTSTR strAudio, WXCTSTR strVideo, WXCTSTR strMixer, int async) {
	WXConvert *obj = (WXConvert *)p;
	return obj ? obj->MixerAV(strVideo, strAudio, strMixer, async) : -1;
}

WXDELOGO_CAPI int WXFfmpegReplaceAudio(void * p, WXCTSTR strVideo, WXCTSTR strAudio, WXCTSTR strOutput, int copy, int async){
    WXConvert *obj = (WXConvert *)p;
    return obj ? obj->ReplaceAudio(strVideo, strAudio, strOutput, copy, async) : -1;
}

WXDELOGO_CAPI int WXFfmpegShotPicture(void * p, WXCTSTR strInput, int64_t ts, WXCTSTR strOutput) {
	WXConvert *obj = (WXConvert *)p;
	return obj ? obj->ShotPicture(strInput, ts, strOutput) : -1;
}

WXDELOGO_CAPI int64_t WXFfmpegGetCurrTime(void * p) {
	WXConvert *obj = (WXConvert *)p;
	return obj ? obj->GetCurrTime() : 0;
}

WXDELOGO_CAPI int64_t WXFfmpegGetTotalTime(void * p) {
	WXConvert *obj = (WXConvert *)p;
	return obj ? obj->GetTotalTime() : 0;
}

WXDELOGO_CAPI int WXFfmpegGetState(void * p) {
	WXConvert *obj = (WXConvert *)p;
	return obj ? obj->GetState() : -1;
}

WXDELOGO_CAPI void WXFfmpegInterrupt(void * p) {
	WXConvert *obj = (WXConvert *)p;
	if (obj)obj->Interrupt();
}


//WXDELOGO_CAPI int  WXFfmpegConvertVideoFast2(WXCTSTR strInput, WXCTSTR strOutput)
//{
//	WXConvert conv;
