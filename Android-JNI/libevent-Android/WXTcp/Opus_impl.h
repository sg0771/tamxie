...
#ifndef _OPUS_IMPL_H_
#define _OPUS_IMPL_H_
...
class WXOpusEncoder {
	...
	int m_nFrameSize = 0;
public:
	WXOpusEncoder(int nSampleRate, int nChannel){
...
static void* WXOpusEncoderCreate(int inSampleRate, int inChannel) {
	WXOpusEncoder *obj = new WXOpusEncoder(inSampleRate, inChannel);
	return (void*)obj;
}

//返回1 表示有编码输出
static int   WXOpusEncoderEncodeFrame(void* ptr, uint8_t *buf_in,  uint8_t **buf_out, int *out_len) {
	WXOpusEncoder *obj = (WXOpusEncoder*)ptr;
	if (obj) {
...
class WXOpusDecoder {
	...
	int m_nFrameSize = 0;
public:
	WXOpusDecoder(int nSampleRate, int nChannel) {
		int error = 0;
