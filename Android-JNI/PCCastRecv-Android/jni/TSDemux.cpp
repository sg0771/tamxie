...
public:
			...
			if (get_stream_data(&pkt))
			{
				if (pkt.size > 0 && pkt.data)
				{
					if (pkt.pid == m_AVContext->m_pidAAC) {
						m_aacBuf.m_iPos = pkt.size;
						memcpy(m_aacBuf.m_pBuf, pkt.data, pkt.size);
						result = TS_TYPE_AUDIO;  //AUDIO
					}
					else 	if (pkt.pid == m_AVContext->m_pidH264) {
						ParseExtraData(pkt.data, pkt.size);
						m_h264Buf.m_iPos = pkt.size;
						memcpy(m_h264Buf.m_pBuf, pkt.data, pkt.size);
						result = TS_TYPE_VIDEO;
					}
				}
			}
		}
	...
	void   GetExtraData(uint8_t*&buf, int &buf_size) {
		buf = m_extradata.m_pBuf;
		buf_size = m_extradata.m_iBufSize;
	}
	void   GetVideoData(uint8_t *&buf, int &buf_size) {
		buf = m_h264Buf.m_pBuf;
		buf_size = m_h264Buf.m_iPos;
		m_h264Buf.m_iPos = 0;//Reset Pos
	}
	void   GetAudioData(uint8_t *&buf, int &buf_size) {
		buf = m_aacBuf.m_pBuf;
		buf_size = m_aacBuf.m_iPos;
		m_aacBuf.m_iPos = 0;
	}
private:
	...
	bool m_bExtra = false;//已经解析SPS+PPS

	WXDataBuffer m_extradata;
	...
	bool ParseExtraData(const uint8_t *buf, size_t buf_size) {
		...
		if (FindSPS && FindPPS) {
			...
			m_extradata.Init(nullptr, sps_len + pps_len);
			memcpy(m_extradata.m_pBuf, buf + sps_pos, sps_len);
			memcpy(m_extradata.m_pBuf + sps_len, buf + pps_pos, pps_len);
			return true;
		}
		return false;
	}
...
extern "C" void* TSDemuxCreate() {
	WXTsDemux *dex = new WXTsDemux();
	return (void*)dex;
}

extern "C" void  TSDemuxDestroy(void *ptr) {
	if (ptr) {
...
extern "C" int  TSDemuxWriteData(void *ptr, const uint8_t *buf, int size) {
	if (ptr) {
		WXTsDemux *dex = (WXTsDemux *)ptr;
...
extern "C" void   TSDemuxGetExtraData(void *ptr, uint8_t **buf, int *buf_size) {
	if (ptr) {
		WXTsDemux *dex = (WXTsDemux *)ptr;
...
extern "C" void   TSDemuxGetVideoData(void *ptr, uint8_t **buf, int *buf_size) {
	if (ptr) {
		WXTsDemux *dex = (WXTsDemux *)ptr;
...
extern "C" void   TSDemuxGetAudioData(void *ptr, uint8_t **buf, int *buf_size) {
	if (ptr) {
		WXTsDemux *dex = (WXTsDemux *)ptr;
