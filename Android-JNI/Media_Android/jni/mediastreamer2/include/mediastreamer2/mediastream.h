...
#if V2_PHONE_SUPPORT
MS2_PUBLIC int media_stream_handle_rtp_v2_p2p_message(MediaStream *stream,mblk_t *m);
MS2_PUBLIC int media_stream_handle_rtcp_v2_p2p_message(MediaStream *stream,mblk_t *m);
#endif
...
struct _VideoStream
{
	...
	#if V2_PHONE_SUPPORT
	bool_t ssrc_changed;
	unsigned int cur_video_ssrc;
	AudioStream *inband_audio_stream;//by wanquan,for ffmpeg-demxu-stream  support
	bool_t stream_ready;//this is for ipcamera ffmpeg stream
	#endif
};
...
#if V2_PHONE_SUPPORT
MS2_PUBLIC int ipcam_stream_start(VideoStream *stream, RtpProfile *profile,const char* recv_fmtp,const char* codec_type,int payload,int jitt_comp);
MS2_PUBLIC void ipcam_stream_stop(VideoStream * stream);
MS2_PUBLIC int  ffmpeg_stream_start(VideoStream *stream, RtpProfile *profile);
MS2_PUBLIC int  ffmpeg_stream_stop(VideoStream *stream);
#endif
...
MS2_PUBLIC void video_stream_refresh_device_for_rotation(VideoStream *stream);
