...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------
//留黑模式 ,
void  xProcess(int src_w, int src_h, int dst_w, int dst_h, int *dw, int *dh) {
	*dw = 0; *dh = 0;
...
static void ConvertData(AndroidRender *render, uint8_t* buf){
	...
	memset(render->m_tmp,0,dst_size);
	memset(render->m_tmp + dst_size,128,dst_size/2);
	...
	pYd + render->m_dh * render->m_win_w + render->m_dw,  render->m_win_w, 
	pUd + (render->m_dh / 2) * render->m_win_w/2 + render->m_dw / 2,  render->m_win_w / 2, 
	pVd + (render->m_dh / 2) * render->m_win_w/2 + render->m_dw / 2,   render->m_win_w / 2, 
	render->m_win_w - 2 * render->m_dw, render->m_win_h - 2 * render->m_dh, 
	kFilterLinear);
}
//---------------------------------------------------------------------------------------------------------------------------------------------------------------
//SurfaceView 在XML里面的大小， 或者上层API获取的大小
JNIEXPORT jint   Java_com_mediastream_MSDraw2_xCreate  ( JNIEnv * env, jobject activity, int src_w, int src_h, int win_w, int win_h){
	...
	LOGE("+++++ MSDraw2_xCreate src=%dx%d win=%dx%d_%dx%d  Delta=%dx%d",
	src_w,src_h, win_w, win_h,  render->m_win_w, render->m_win_h, render->m_dw, render->m_dh
	);
	return (jint)render;
}

JNIEXPORT void Java_com_mediastream_MSDraw2_xDestroy( JNIEnv * env, jobject activity, jint handle){
	if(handle == 0)return;
...
JNIEXPORT void Java_com_mediastream_MSDraw2_xDraw   ( JNIEnv * env, jobject activity, jint handle, jobject surface, jbyteArray buf){
	if(handle == 0)return;
	AndroidRender *render = (AndroidRender*)handle;
	//Clip YV12 Data
