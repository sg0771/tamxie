...
public://WXString
    ...
    WXString& operator+=(const char*  sz) {
...
public:
	int m_nMaxError = 10;//最多错误次数，超过之后内部log不执行
	int m_nError = 0;
	...
	int Read(uint8_t *pBuf, int nSize) {//读数据
		...
		if (nSize && nowSize >= nSize) { //数据足够读
			if (pBuf) {
				...
				if (posLeft >= nSize) { //数据区足够
					memcpy(pBuf, m_dataBuffer.m_pBuf + posRead, (size_t)nSize);
				}
				else {  //有部分数据需要写到RingBuffer的头部
					memcpy(pBuf, m_dataBuffer.m_pBuf + posRead, (size_t)posLeft);//从尾部拷贝数据
					memcpy(pBuf + posLeft, m_dataBuffer.m_pBuf, (size_t)(nSize - posLeft));//从头部拷贝数据
				}
			}
			m_nPosRead += nSize;
			m_last = pBuf[nSize - 1];
...
#define TYPE_DISCONNECT 1
#define TYPE_VIDOE_DATA 2
#define TYPE_AUDIO_DATA 3
typedef void (* PCCastOnData)(void *pSink,uint64_t uid, int type, uint8_t* buf, int buf_size);
