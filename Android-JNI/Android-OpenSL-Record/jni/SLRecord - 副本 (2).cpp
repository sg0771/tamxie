...
void WXSLRecordOnWrite(const char *buf, int bufSize);
class SLRecord {
public:
	...
	void CbFunc() {
		...
		if(m_fifo.Size() >= 2048){
			...
			if(aacSize > 0 ){
				WXSLRecordOnWrite((const char*)aacData, aacSize);
...
typedef struct jni_context {
    JavaVM *javaVM;
    jclass wxSLRecordClz;
    jobject wxSLRecordObj;
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass clz = env->FindClass("com/apowersoft/SLRecord");
    g_ctx.wxSLRecordClz = (jclass) env->NewGlobalRef(clz);

    jmethodID wxSLRecordCtor = env->GetMethodID(g_ctx.wxSLRecordClz, "<init>", "()V");
    jobject handler = env->NewObject(g_ctx.wxSLRecordClz, wxSLRecordCtor);
    g_ctx.wxSLRecordObj = env->NewGlobalRef(handler);
...
void WXSLRecordOnWrite(const char *buf, int bufSize) {
    ...
    jmethodID callbackFunc = env->GetMethodID(g_ctx.wxSLRecordClz, "WXSLRecordOnWrite", "([BI)V");
    if (!callbackFunc) {
        LOGE("Failed to retrieve WXSLRecordOnWrite() methodID @ line %d", __LINE__);
        return;
    }

    env->CallVoidMethod(g_ctx.wxSLRecordObj, callbackFunc, jarrayBuf);
