...
class Encoder {
	...
	int m_nChannels/* = 2*/;
	faacEncHandle m_h/* = NULL*/;//编码器
	uint32_t m_nMax/* = 0*/;//最大输出长度
	uint32_t m_samplesInput/* = 0*/;
	uint8_t m_pBuf[IMAXSIZE];//输出
public:
	int Open(int sample_rate /*= 44100*/, int channels /*= 2*/, int bitrate /*= 96*/) {
		...
		cfg->inputFormat = FAAC_INPUT_16BIT;//输入数据格式
	...
	int  Encode(uint8_t* pBuf, uint8_t** pOut, int* iOutSize) {
		if (m_h) {
			int index = 3;
			do {
				int size = faacEncEncode(m_h, (int32_t*)pBuf, m_samplesInput, m_pBuf, m_nMax);//编码器
				if (size > 0) {
					*iOutSize = size;
					*pOut = m_pBuf;
					return 1;
...
extern "C" void* FaacEncoderOpen(int sample_rate /*= 44100*/, int channels /*= 2*/, int bitrate /*= 96*/) {
	LibFaac::Encoder* obj = new LibFaac::Encoder();
	int ret = obj->Open(sample_rate, channels, bitrate);
	if (ret > 0) {
		return (void*)obj;
	}
	delete obj;
...
extern "C" int  FaacEncoderEncode(void* ptr, uint8_t* pBuf, uint8_t** pOut, int* iOutSize) {
	if (ptr) {
		LibFaac::Encoder* obj = (LibFaac::Encoder*)ptr;
		return obj->Encode(pBuf, pOut, iOutSize);
	}
	return 0;
}

extern "C" void FaacEncoderClose(void* ptr) {
	if (ptr) {
		LibFaac::Encoder* obj = (LibFaac::Encoder*)ptr;
		obj->Close();
		delete obj;
