...
#if defined(TEMPLATE_DITHER_FLT)
...
void RENAME(swri_noise_shaping)(WXTcpSwrContext *s, AudioData *dsts, const AudioData *srcs, const AudioData *noises, int count){
    ...
    assert((taps&3) != 2);
    assert((taps&3) != 3 || s->dither.ns_coeffs[taps] == 0);
