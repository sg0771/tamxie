...
static void ms_ticker_init(MSTicker *ticker, const MSTickerParams *params)
{
	...
	ticker->ticks=1;
	ticker->time=0;
	ticker->interval=params->iduration/*TICKER_INTERVAL*/;
	ticker->run=FALSE;
	ticker->exec_id=0;
...
MSTicker *ms_ticker_new(int iduration){
	MSTickerParams params;
	params.iduration = iduration;
...
#if V2_PHONE_SUPPORT
int ms_ticker_attach_single(MSTicker *ticker, MSFilter *f){
	MSList *sources=ms_list_append(NULL,f);
	ms_filter_preprocess((MSFilter*)f,ticker);
...
#if V2_PHONE_SUPPORT
int ms_ticker_detach_single(MSTicker *ticker, MSFilter *f){
	if (f->ticker==NULL) {
		ms_message("Filter %s is not scheduled; nothing to do.",f->desc->name);
		return 0;
	}
	ms_mutex_lock(&ticker->lock);
	ticker->execution_list=ms_list_remove(ticker->execution_list,f);
	ms_mutex_unlock(&ticker->lock);
	call_postprocess(f);
	ms_message("ms_ticker_detach_single leave");
	return 0;
}
#endif
...
int ms_ticker_detach(MSTicker *ticker,MSFilter *f){
	ms_message("ms_ticker_detach enter");
	MSList *sources=NULL;
	MSList *filters=NULL;
	MSList *it;
	...
	ms_message("ms_ticker_detach leave");
	return 0;
}
...
void * ms_ticker_run(void *arg)
{
	...
	while(s->run){
		...
		if (late>s->interval*5 && late>lastlate){
			//ms_warning("%s: We are late of %d miliseconds.",s->name,late); //by wanquan
		}
		lastlate=late;
		ms_mutex_lock(&s->lock);
	}
