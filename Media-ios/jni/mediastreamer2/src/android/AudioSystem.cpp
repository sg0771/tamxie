...
AudioSystemImpl::AudioSystemImpl(Library *lib) :
	...
	mApi18=false;//merge from opensource-13-12-13,by wanquan
	...
	if (!mGetOutputSamplingRate.isFound()) {
		...
		if (!mGetOutputSamplingRate.isFound()){//merge from opensource-13-12-13,by wanquan
	...
	if (!mGetOutputFrameCount.isFound()) {
		...
		if (!mGetOutputFrameCount.isFound())//merge from opensource-13-12-13,by wanquan
...
bool AudioSystemImpl::init(Library *lib){//merge from opensource-13-12-13,by wanquan
	bool fail=false;//merge from opensource-13-12-13,by wanquan
