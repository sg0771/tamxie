...
namespace fake_opensles {
	...
	int initOpenSLES() {
		...
		ms_message("initOpenSLES Success");
...
int gs_nReplayLastPkgCounts = 0;
int gs_nMaxReplayCounts = 16;//5
int gs_nBufferSize = 0;
...
static void android_snd_card_detect(MSSndCardManager *m) {
	...
	if (sdk_version >= 14) { // Use only if Android OS >= KIT_KAT (4.4),
	//if (sdk_version >= 20) { // Use only if Android OS >= KIT_KAT (4.4), by wanquan,we need test in future
		...
		ms_warning("Android version is %i, OpenSLES MS soundcard unavailable on Android < 4.0", sdk_version);
	}
}
...
static void compute_timespec(OpenSLESInputContext *ictx) {
	...
	double av_skew = ictx->mTickerSynchronizer != NULL ? ms_ticker_synchronizer_set_external_time(ictx->mTickerSynchronizer, &ts) : 0.0;
	if ((++count) % 100 == 0)
...
static int GetMaxAudio(int16_t *pcm, int len){
	...
	int i;
	for(i = 0; i < len ;i ++){
		if(abs(pcm[i] > MaxValue))MaxValue = abs(pcm[i]);
	}
	if(MaxValue < 300)
		memset(pcm,0,len*2);
	return MaxValue;
...
static void opensles_recorder_callback(SLAndroidSimpleBufferQueueItf bq, void *context) {
	...
	if(ictx->q.q_mcount < 20)
		putq(&ictx->q, m);
	ms_mutex_unlock(&ictx->mutex);
	
	gCaptureMax = GetMaxAudio((int16_t*)  ictx->recBuffer[ictx->currentBuffer], 800);
	//ms_error("gCaptureMax = %d",gCaptureMax);
...
static void opensles_player_callback(SLAndroidSimpleBufferQueueItf bq, void* context) {
	...
	if(octx->nbufs == 0) (*octx->playerBufferQueue)->Clear(octx->playerBufferQueue); //Çå¿ÕÉù¿¨»º³å¶ÓÁÐ add

	if (avail >= ask) {
		ask = ms_bufferizer_read(&octx->buffer, octx->playBuffer[octx->currentBuffer], ask);
		gs_nBufferSize = avail / ask - 1;
	} else{
//		gs_nReplayLastPkgCounts++;
//		if(gs_nReplayLastPkgCounts > gs_nMaxReplayCounts){
			memset(octx->playBuffer[octx->currentBuffer], 0, ask);
//		}
	}
	ms_mutex_unlock(&octx->mutex);
	octx->nbufs++;
 	result = (*octx->playerBufferQueue)->Enqueue(octx->playerBufferQueue, octx->playBuffer[octx->currentBuffer], ask);
