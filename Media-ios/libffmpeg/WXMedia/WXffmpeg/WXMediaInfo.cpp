...
public:
	...
	DataFrame *m_frame = NULL;// attach 的数据帧
...
WXFFMPEG_CAPI int  WXMediaInfoGetAudioChannelNumber(void *p) {
	WXMediaInfo *info = (WXMediaInfo*)p;
	if (info) {
...
WXFFMPEG_CAPI int    WXMediaInfoGetAttachChannelNumber(void *p) {
	WXMediaInfo *info = (WXMediaInfo*)p;
	if (info) {
...
WXFFMPEG_CAPI int   WXMediaInfoGetVideoChannelNumber(void *p) {
	WXMediaInfo *info = (WXMediaInfo*)p;
	if (info) {
...
WXFFMPEG_CAPI void*  WXMediaInfoCreate(WXCTSTR wszFileName, int *error) {
	WXString wxstr = wszFileName;
	struct stat st;
	int ret = stat(wxstr.c_str(), &st);

	AVFormatContext *ic = NULL;
	int err = avformat_open_input(&ic, wxstr.c_str(), NULL, NULL);//打开文件
	if (err < 0) {
	...
	if (ic->nb_streams) {	
		...
		info->m_strFormat.Format("%s",ic->iformat->long_name);
		for (int i = 0; i < ic->nb_streams; i++) {
			if (ic->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
                ...
                if (ic->streams[i]->disposition & AV_DISPOSITION_ATTACHED_PIC) {
					...
					if (pkt.size) {
						cinfo->m_frame = new DataFrame(pkt.data, pkt.size);
					}
				}else {
					cinfo->m_iType = 1;
...
WXFFMPEG_CAPI int WXMediaInfoChannelGetAttachSize(void *p, int index) {
	WXMediaInfo *info = (WXMediaInfo*)p;
...
WXFFMPEG_CAPI uint8_t*       WXMediaInfoChannelGetAttachData(void *p, int index) {
	WXMediaInfo *info = (WXMediaInfo*)p;
	if (info && info->m_arrInfo[index]->m_iType == 2) {
...
WXFFMPEG_CAPI int WXMediaInfoChannelGetType(void *p, int index) {
	WXMediaInfo *info = (WXMediaInfo*)p;
	if (info) {
...
//有的ret返回0，但是没有生成对应文件！！
WXFFMPEG_CAPI int   WXMediaInfoGetPicture(void *p, WXCTSTR wszFileName) {
