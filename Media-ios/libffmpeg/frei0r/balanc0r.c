...
typedef struct balanc0r_instance
{
	unsigned int width;
	unsigned int height;
...
static void f0r_get_param_info(f0r_param_info_t* info, int param_index)
{
	switch(param_index) {
		case 0:
			...
			break;
		case 1:
			info->name = "Green Tint";
			info->type = F0R_PARAM_DOUBLE;
			info->explanation = "Adjust the level of green.";
			break;
	}
...
static void setRGBmult(balanc0r_instance_t *o)
{
	int   t = o->temperature/10.0 - 200.0;
	float mi;

	o->mr  = 1.0 / bbWB[t][0];
	o->mg  = 1.0 / bbWB[t][1] * o->green;
	o->mb  = 1.0 / bbWB[t][2];
		...
		f0r_param_t param, int param_index)
{
	...
	switch(param_index) {
		case 0: {
			...
			int    m = (l + r )/2;
			double t = MAX(inst->color.r, inst->color.g);

			t = MAX(t, inst->color.b);
			if (t > 0) {
				...
				t = m * 10.0 + 2000.0;
				inst->temperature = CLAMP(t, 2200.0, 7000.0);
				inst->green = (bbWB[m][1] / bbWB[m][0]) / (sG/sR);
			}
			setRGBmult(inst);
			break;
		}
