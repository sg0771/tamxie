...
/*
		const uint8_t* src2 = (uint8_t*)(&inframe[x+inst->width*y]);
                ...
                const uint32_t* inframe, uint32_t* outframe)
{
  ...
  for (x=0; x < inst->width; ++x) {
	  ...
	  const uint8_t* src2 = (uint8_t*)( &inframe[x+inst->width*y]);
	  uint8_t* dst2 		= (uint8_t*)(&outframe[x+inst->width*y]);
  ...
  for (k = 0; k < inst->num; k++) {
	  ...
	  if (cc->numpix > 0) {
		  ...
		  cc->r = (uint8_t) (cc->aggr_r/cc->numpix);
		  cc->g = (uint8_t) (cc->aggr_g/cc->numpix);
		  cc->b = (uint8_t) (cc->aggr_b/cc->numpix);
