...
//--------------------------------------------------
static void f0r_get_param_value(f0r_instance_t instance, f0r_param_t parm, int param_index)
{
	param *p;
	float tmpf;
...
static void f0r_update(f0r_instance_t instance, double time, const uint32_t* inframe, uint32_t* outframe)
{
	...
	remap32(p->w, p->h, p->w, p->h, (uint8_t*) inframe, (uint8_t*) outframe, p->map, 0, p->interpol);
