...
static int premica2d(tocka2 t1, tocka2 t2, premica2 *p)
{
	float dx,dy,m;
...
static float razd_t_p(tocka2 t, premica2 p)
{
	float r;
...
static int presecisce2(premica2 p1, premica2 p2, tocka2 *t)
{
	float d1,d2,d3;
...
static void cetverokotnik4(int wi, int hi, int wo, int ho, tocka2 vog[], int str, float strx, float stry, float *map)
{
...
static void trikotnik1(int wi, int hi, int wo, int ho, tocka2 vog[], tocka2 R, tocka2 S, premica2 p12, premica2 p23, premica2 p34, premica2 p41, int t12, int t23, int str, float strx, float stry, float *map)
{
	int x,y;
	tocka2 T,A,B;
	premica2 p5,p6;
...
static void make_alphamap(uint8_t *amap, tocka2 vog[], int wo, int ho, float *map, float feath, int nots[])
{
...
//-------------------------------------------------------
static void apply_alphamap(uint32_t* frame, int w, int h, uint8_t *amap, int operation)
{
	int i,j, length;
	uint32_t t;
        length = w * h;
...
static int geom4c_b(int wi, int hi, int wo, int ho, tocka2 vog[], int str, float strx, float stry, float *map, int nots[])
{
	premica2 p12,p23,p34,p41;
	tocka2 R,S;
...
//-------------------------------------------------------
static interpp set_intp(inst p)
{
...
static f0r_instance_t f0r_construct(unsigned int width, unsigned int height)
{
	...
	in->amap=(uint8_t*)calloc(1, sizeof(char)*(in->w*in->h*2+2));
...
static void f0r_update(f0r_instance_t instance, double time, const uint32_t* inframe, uint32_t* outframe)
{
	...
	remap32(p->w, p->h, p->w, p->h, (uint8_t*) inframe, (uint8_t *) outframe, p->map, bkgr, p->interp);
