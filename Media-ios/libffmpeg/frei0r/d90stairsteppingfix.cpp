...
#else
			float *filled = (float *)alloca(newHeight*sizeof(float));
#endif
            int count = 0;
            int index = 0;
...
#else
			float *downScaling = (float *)alloca(height * sizeof(float));
#endif
...
//          printf("Converting.\n");
            ...
            for (int line = 0; line < height; line++) {
                ...
                cvA = (uint8_t*) &in[width*index];
                cvB = (uint8_t*) &in[width*(index+1)];
                cvOut = (uint8_t*) &out[width*line];
...
filter_dest_cpp(d90stairsteppingfix)