...
static void f0r_get_plugin_info(f0r_plugin_info_t* curves_info)
{
}
...
void updateBsplineMap(f0r_instance_t instance)
{
...
#else 
	bspline_point *points = malloc(count * sizeof(bspline_point));
#endif
    ...
    for (int i = 0; i < count - 1; ++i) {
        ...
        position curve[c];
#else
		position *curve = malloc(c*sizeof(position));
#endif
        while (t <= 1) {
