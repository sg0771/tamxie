...
static void f0r_get_plugin_info(f0r_plugin_info_t* saturat0r_info)
{
}
                ...
                const uint32_t* inframe, uint32_t* outframe)
{
  ...
  if (0 <= saturation && saturation <=1) // optimisation: no clamping needed
  {
    while (len--)
    {
      ...
      *dst++ = (uint8_t) (bw + b*saturation);
      *dst++ = (uint8_t) (bw + g*saturation);
      *dst++ = (uint8_t) (bw + r*saturation);
