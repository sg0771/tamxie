...
//----------------------------------------------------------
static void draw_rectangle(uint8_t *sl, int w, int h, int x, int y, int wr, int hr, uint8_t gray)
{
int i,j;
int zx,kx,zy,ky;
...
void draw_circle(uint8_t *sl, int w, int h, float ar, int x, int y, int rn, int rz, uint8_t gray)
{
int i,j;
int zx,kx,zy,ky;
float rr,rmin,rmax;
...
kx=x+rz/ar+1;  if (kx>=w) kx=w-1;
ky=y+rz+1;  if (ky>=h) ky=h-1;
rmin=(float)rn;
rmax=(float)rz;
for (i=zy;i<ky;i++)
	for (j=zx;j<kx;j++)
...
void draw_wedge(uint8_t *sl, int w, int h, int x, int y, int size, int dir, uint8_t gray)
{
int i,j,ii,jj;
...
switch (dir)
	...
	case 1:		//up
		for (i=0;i<size;i++)
			for (j=0;j<=i;j++)
				{
				ii=y+i; if (ii>=h) ii=h-1;
				jj=x+j; if (jj>=w) jj=w-1;
				sl[w*ii+jj]=gray;
				jj=x-j; if (jj<0) jj=0;
				sl[w*ii+jj]=gray;
				}
...
void sah1(uint8_t *sl, int w, int h, int size, float ar, int rim)
{
int i,j,kx,ky,z,pv,ps,zv,zs;
uint8_t black,gray1,gray2,white;
int ox,oy;
...
oy=ky*2-(h/2)%(ky*2);
ps=(w/2)%kx; if (ps==0) ps=kx;
pv=(h/2)%ky; if (pv==0) pv=ky;
zv=h-pv;
zs=w-ps;
...
void hlines(uint8_t *sl, int w, int h, int size1, int size2, float ar, int clr)
{
int i,iz;
uint8_t black,white;
...
//clr=clear background
void vlines(uint8_t *sl, int w, int h, int size1, int size2, float ar, int clr)
{
int i,iz;
uint8_t black,white;
...
void mreza(uint8_t *sl, int w, int h, int size1, int size2, float ar)
{
...
void pike(uint8_t *sl, int w, int h, int size1, int size2, float ar)
{
int i,j,iz,jz;
int black,white;
int size1a,size2a;
...
void tarca(uint8_t *sl, int w, int h, int size1, int size2, float ar)
{
uint8_t black,white;
int i;
...
void robovi(uint8_t *sl, int w, int h)
{
int i,j,l;
uint8_t black,white;
...
void rulers(uint8_t *sl, int w, int h, uint8_t *a)
{
int i;
uint8_t black,white,tr;
...
//*a = alpha channel
static void grid(uint8_t *sl, int w, int h, uint8_t *a)
{
int i,j;
uint8_t black,white,tr;
...
for (i=0;i<h/2;i++)
	{
	for (j=0;j<w/2;j++) sl[i*w+j]=c1;
	for (j=w/2;j<w;j++) sl[i*w+j]=c2;
	}
for (i=h/2;i<h;i++)
	{
	for (j=0;j<w/2;j++) sl[i*w+j]=c3;
	for (j=w/2;j<w;j++) sl[i*w+j]=c4;
	}
...
static f0r_instance_t f0r_construct(unsigned int width, unsigned int height)
{
  ...
  inst->par=1.0;
  inst->sl=(uint8_t*)calloc(width*height,1);
  inst->alpha=(uint8_t*)calloc(width*height,1);
